  ...
  for (let i = window; i < priceSeries.length; i++) {
    ...
    if (currentPrice.isLessThanOrEqualTo(minPrice.plus(priceRange.times(0.05)))) {
      const supportCount = windowPrices.filter((price) =>
        new BigNumber(price).minus(minPrice).abs().isLessThanOrEqualTo(priceRange.times(0.05))
      ).length;
      ...
      const resistanceCount = windowPrices.filter((price) =>
        maxPrice.minus(new BigNumber(price)).abs().isLessThanOrEqualTo(priceRange.times(0.05))
      ).length;
